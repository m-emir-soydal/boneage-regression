...
def make_seed_split(seed: int, data_fraction: float = 1.0):
    ...
    50 % train / 25 % val / 25 % calib (stratified by sex, random_state=seed);
    test is the external ``test.csv``. Targets are normalized against the train
    split's max bone age (same convention as ``data_loader.load_data``).
    """
