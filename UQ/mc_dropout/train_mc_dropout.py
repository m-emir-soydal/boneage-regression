...
def selection_score(select_by, val_mae, uq, mae_weight):
    ...
    ``uq`` may be ``None`` on epochs where MC eval was skipped; in that case
    only ``val_mae`` selection can produce a score.
    """
    ...
    if select_by == "mpiw90":
        return uq["mpiw90"]
    if select_by == "combo":
        # Calibration already fixes coverage, so minimize calibrated width with
        # a penalty that keeps point accuracy from degrading.
...
def main():
    ...
    for epoch in range(epochs):
        ...
        # Standard (dropout-off) validation MAE.
        model.eval()
        val_loss = 0.0
        with torch.no_grad():
        ...
        # UQ-aware calib eval (MC Dropout + post-hoc temperature).
    ...
    # run_mc_dropout.py --calibration auto can find it.
