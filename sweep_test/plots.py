...
def _gap_pinaw_panel(summary, confidence, fig_dir):
    part_conf = summary[summary["confidence"].round(6) == round(confidence, 6)]
    ...
    _save(fig, fig_dir / f"gap_pinaw_vs_cal_fraction_{suffix}")
...
def build_plots(input_dir, plot_confidence, calibration_curve_budgets):
    ...
    _gap_pinaw_panel(summary, plot_confidence, fig_dir)
...
def main():
    ...
    parser.add_argument("--plot-confidence", type=float, default=S.PLOT_CONFIDENCE)
    parser.add_argument(
    ...
    build_plots(args.input_dir, args.plot_confidence, args.calibration_curve_budgets)
